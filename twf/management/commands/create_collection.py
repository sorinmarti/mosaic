    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            collection = Collection(
                title="Songs", description="A collection of songs", project=project
            )
        ...
        for document in Document.get_active_documents(project):
            logger.debug(
                "Processing document: %s, document_id: %s",
                document,
            ...
            for page in document.get_active_pages():
                ...
                for annotation in annotations:
                    if "type" not in annotation:
                        logger.warning(
                            "Skipping annotation without type: %s", annotation
                        )
                        continue
                    anno_types.append(annotation["type"])
                    if annotation["type"] in ["lyrics", "music", "heading"]:
                        collection_item.document_configuration["annotations"].append(
                            annotation
                        )
            ...
            created_col_items += 1
            logger.info(
                "Added Song for document %s with %d annotations.",
                document.document_id,
                len(collection_item.document_configuration["annotations"]),
            )
