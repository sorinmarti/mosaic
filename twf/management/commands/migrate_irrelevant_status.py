...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'DRY RUN: Would migrate {count} documents from irrelevant to needs_tk_work')
            )
        ...
        migration_note = 'Previously marked as irrelevant - needs Exclude label in Transkribus'
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully migrated {count} documents from irrelevant to needs_tk_work')
        )
